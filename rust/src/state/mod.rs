...
use crate::{
    ...
    utility::{
        functions::{extract_network_height_hash, pretty_print_duration},
        store::{
...
pub struct IndexerState {
    ...
    pub staking_ledgers: HashSet<(u32, LedgerHash)>,
...
impl IndexerState {
    ...
    pub async fn add_startup_staking_ledgers_to_store(&mut self, ledgers_dir: &Path) -> Result<()> {
        ...
        if let Some(indexer_store) = self.indexer_store.as_ref() {
            ...
            let tasks: Vec<_> = ledger_parser
                .ledger_paths
                .map(|path| {
                    let mut staking_ledgers = self.staking_ledgers.clone();
    ...
    pub async fn process_staking_ledger(
        ...
        let (network, epoch, hash) = extract_network_height_hash(path);
            ...
            .is_none()
        {
            ...
            debug!(
                "Skipping staking ledger {}-{}-{}. Already present in store.",
                network, epoch, hash
            )
        }
    ...
    /// Compact the indexer store at the specified cadenece
    fn compact_db_every_n_blocks(&self, cadence: u32) -> Result<()> {
        if self.blocks_processed % cadence == 0 {
            if let Some(store) = self.indexer_store.as_ref() {
                let file = tempfile::NamedTempFile::new()?;

                store.create_snapshot(file.path())?;
