...
impl StagedLedgerStore for IndexerStore {
    ...
    fn add_staged_ledger_at_state_hash(
        ...
        } else if state_hash.0 == DEVNET_GENESIS_PREV_STATE_HASH
            && self
                .add_staged_ledger_hash(
                    &LedgerHash::new_or_panic(DEVNET_GENESIS_LEDGER_HASH.into()),
                    state_hash,
                )
                .unwrap_or(false)
        {
            self.add_event(&IndexerEvent::Db(DbEvent::Ledger(
                DbLedgerEvent::NewLedger {
                    blockchain_length: DEVNET_GENESIS_BLOCKCHAIN_LENGTH - 1,
                    state_hash: state_hash.clone(),
